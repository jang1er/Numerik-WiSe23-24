...
xkNewton = newtonSkalar(f, df, 3, 1e-14, 100);
temp = xkNewton;

nBisek = size(xkBisek,1)/3;
nFalsi = size(xkFalsi,1)/3;
nSek = size(xkSek,1)/3;
nNewton = size(xkNewton,1)/3;
...
errorNewton = abs(xkNewton(xi(0:nNewton-1)) - x);
figure;
hold on;
semilogy(errorBisek);
...
semilogy(errorFalsi);
semilogy(errorSek);
semilogy(errorNewton);
set(gca, 'yscale','log');
hold off;
legend('Error Bisection','Bisek:Interval length','Error Falsi','Error Sekanten', 'Error Newton')
...
xkNewton = newtonSkalar(f, df, 3, 1e-14, 300);
